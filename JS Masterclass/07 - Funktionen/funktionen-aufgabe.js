...
function getSmallestCourse() {
  ...
  for (const i in courses) {
    ...
    if (courses[i].length < courses[smallestCourse].length) {
      smallestCourse = i
    }
  }
...
console.log("Der Kurs", getSmallestCourse(), "ist der kleinste Kurs.")
...
function translateWord(word) {
  ...
  const wordIndex = LANGUAGE_DE.indexOf(word)
  if (!LANGUAGE_DE.includes(word)) {
    return translatedWord
  }
  translatedWord = LANGUAGE_EN[wordIndex]
  return translatedWord
}
...
function translateSentence(sentence) {
  ...
  for (const i in sentenceWords) {
    let translatedWord

    if (i == 0) {
      translatedWord = ucFirst(translateWord(sentenceWords[i]))
    } else {
      translatedWord = translateWord(sentenceWords[i])
    }
